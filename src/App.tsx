...
import useAxiosFetch, { base } from './hooks/useAxiosFetch'
...
export const App = () => {
  ...
  const { response: data } = useAxiosFetch('/api/admission/form')
  ...
  const onFormSubmit = async () => {
    ...
    if (isValid) {
      setLoad(true)
      try {
        const { data: res } = await base.post('/api/applicant/check', formInput)
        if (res?.responseCode === 200) {
          payWithPayStack({
            onClose: () => {
              swal('Payment Declined', 'Payment process with PayStack has been declined', 'warning').then(() => window.location.reload())
            },
            onSuccess: (val) => createNewApplicant(val)
          })
        }
      } catch (error: any) {
        swal('Duplicate', error?.response?.data?.message, 'error')
      } finally {
        setLoad(false)
      }
    }
  ...
  const createNewApplicant = async (val: any) => {
    // console.log('PAYSTACK SUCCESS ==>', val)
    try {
      setOpen(true)
      const { data: res } = await base.post('/api/applicant/new', formInput)
      if (res?.responseCode === 200) {
        dispatch({ type: 'RESET' })
        setOpen(false)
        await swal({
  ...
  return (
    <>
      ...
      {/* Loader */}
      <Dialog open={open} maxWidth={'xs'} fullWidth sx={{ backdropFilter: 'blur(5px)' }}>
        <DialogContent sx={{ textAlign: 'center', p: 5 }}>
          <Typography mb={1}>{'Processing payment. Please wait...'}</Typography>
